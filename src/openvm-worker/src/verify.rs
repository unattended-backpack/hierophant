//! Verification of OpenVM proofs, ported verbatim from the hierophant's
//! in-process `openvm/verify.rs` at the worker split.
//!
//! Trust model parity with the SP1 + Bonsai paths: the caller re-derives
//! every key and commitment from the client-uploaded ELF + app config, so a
//! worker can neither substitute a different program nor return a
//! structurally valid proof of the wrong execution. A failed verification
//! strikes/drops the worker (handled by the hierophant's router).
//!
...
//! from the universal internal-recursive artifact `cargo openvm setup` writes
//! to ~/.openvm/internal_recursive.pk (plus an in-process prefix keygen) when
...
pub fn verify_openvm_proof(
    ...
    match mode {
        ...
        ProofMode::Evm => {
            ...
            // this lean verify path does not carry. EVM proofs are made
...
// Mirror of the proving side's config construction (which itself mirrors the
...
fn ensure_agg_pk(
    ...
    let Some(internal_recursive) = try_load_internal_recursive_pk() else {
        warn!(
            "No pre-generated OpenVM internal-recursive key found at ~/.openvm/internal_recursive.pk; running full aggregation keygen in-process. This is slow and RAM-heavy; run `cargo openvm setup` on this host to avoid it."
        );
        return Ok(None);
    };
