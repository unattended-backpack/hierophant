...
impl ProverNetwork for ProverNetworkService {
    ...
    async fn request_proof(
        ...
        // Check if we need circuit artifacts for this proof mode
        let needs_artifacts = matches!(mode, ProofMode::Groth16 | ProofMode::Plonk);
        let artifacts_ready = if needs_artifacts {
            check_artifacts_ready(mode)
        } else {
            true
        };
        ...
        if artifacts_ready {
            // Artifacts ready (or not needed), route the proof to a worker immediately
            if let Err(e) = self
                .state
                .proof_router
                .route_proof(
            ...
            // Artifacts not ready, store as pending and start download
            info!("Circuit artifacts not ready for proof {request_id}, starting download");
            self.state
            ...
            // Spawn task to download artifacts and route proof when ready
            let state = self.state.clone();
            tokio::spawn(async move {
                download_artifacts_and_route_proof(state, request_id, program_uri, stdin_uri, mode)
                    .await;
            });
        }
    ...
    async fn get_proof_request_status(
        ...
        // Check if proof is waiting for circuit artifacts to download
        if self
            .state
            .proofs_pending_artifacts
            .lock()
            ...
            .contains(&request_id)
        {
            info!("Proof {request_id} is waiting for circuit artifacts to download");
...
fn check_artifacts_ready(mode: ProofMode) -> bool {
    match mode {
        ...
        _ => true, // Other modes don't need artifacts
    }
}

/// Download circuit artifacts and route proof when complete
async fn download_artifacts_and_route_proof(
    state: Arc<HierophantState>,
    request_id: B256,
    ...
    let artifact_type = match mode {
        ...
        _ => {
            error!("download_artifacts_and_route_proof called with mode that doesn't need artifacts: {}", mode.as_str_name());
            return;
        }
    };

    info!(
        "Starting download of {} circuit artifacts for proof {request_id}",
        artifact_type
    );
    ...
    if let Err(e) = std::fs::write(&marker_file, "") {
        error!(
            "Failed to create {} download completion marker: {e}",
            artifact_type
        );
    ...
    info!(
        "Downloaded {} circuit artifacts to {}",
        artifact_type,
