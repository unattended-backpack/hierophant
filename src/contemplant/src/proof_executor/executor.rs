...
use crate::worker_state::ActiveProver;
...
pub async fn execute_proof(
    ...
    let progress_tracking_available = state.progress_tracking_available;
    tokio::task::spawn(async move {
        ...
        let (pk, _) = if mock {
            ...
            match &state.active_prover {
                ActiveProver::Cpu(cpu_prover) => cpu_prover.setup(&proof_request.elf),
        ...
        let proof_res = match proof_request.mode {
            ...
            ProofMode::Core => {
                if mock {
                    ...
                    match &state.active_prover {
                        ActiveProver::Cpu(cpu_prover) => cpu_prover.prove(&pk, stdin).core().run(),
                        ActiveProver::Cuda(cuda_prover) => cuda_prover.prove(&pk, stdin).core().run(),
                    }
                }
            }
            ProofMode::Compressed => {
                if mock {
                    ...
                    match &state.active_prover {
                        ActiveProver::Cpu(cpu_prover) => cpu_prover.prove(&pk, stdin).compressed().run(),
                        ActiveProver::Cuda(cuda_prover) => cuda_prover.prove(&pk, stdin).compressed().run(),
                    }
                }
            }
            ProofMode::Plonk => {
                if mock {
                    ...
                    match &state.active_prover {
                        ActiveProver::Cpu(cpu_prover) => cpu_prover.prove(&pk, stdin).plonk().run(),
                        ActiveProver::Cuda(cuda_prover) => cuda_prover.prove(&pk, stdin).plonk().run(),
                    }
                }
            }
            ProofMode::Groth16 => {
                if mock {
                    ...
                    match &state.active_prover {
                        ActiveProver::Cpu(cpu_prover) => cpu_prover.prove(&pk, stdin).groth16().run(),
                        ActiveProver::Cuda(cuda_prover) => cuda_prover.prove(&pk, stdin).groth16().run(),
                    }
                }
            }
        };
