...
pub(super) async fn start_assessor(
    ...
    request_id: B256,
    progress_tracking_available: bool,
) -> Result<()> {
    info!("Starting assessor...");

    // Only generate execution report if we can actually track progress
    if progress_tracking_available {
        // Send initial progress update immediately so Hierophant knows work has started
        // This prevents timeout during mock execution phase
        proof_store_client
            .proof_progress_update(request_id, ProgressUpdate::Execution(1))
            .await;
        ...
        info!("Progress tracking unavailable - skipping execution report and sending static progress update");
        // Send an initial progress update so Hierophant knows work has started
        // Must be non-zero since Hierophant only updates if progress_update > current_progress
        proof_store_client
            .proof_progress_update(request_id, ProgressUpdate::Execution(1))
            .await;

        // Keep shutdown_rx alive so the channel doesn't close prematurely
        tokio::spawn(async move {
